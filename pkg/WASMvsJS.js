...
* @returns {Uint32Array}
*/
export function wasm_sieve(limit) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        wasm.wasm_sieve(retptr, limit);
...
* @param {number} size
*/
export function wasm_ram_test(size) {
    wasm.wasm_ram_test(size);
}

/**
* @param {number} count
*/
export function wasm_create_elements(count) {
    wasm.wasm_create_elements(count);
}
...
async function __wbg_init(input) {
    ...
    if (typeof input === 'undefined') {
        input = new URL('WASMvsJS_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
